...
- 로그인된 Codex CLI의 App Server를 shell 없이 stdio로 실행하고 `initialize`/`initialized` 뒤 `account/usage/read`를 호출한다.
- 기본 실행 파일은 Windows `codex.exe`, macOS/Linux `codex`이며 절대 경로 `AGENT_CARD_CODEX_BIN` override만 허용한다.
- `dailyUsageBuckets[].startDate/tokens`, `summary.lifetimeTokens`만 sanitizer가 읽는다.
- timeout, 인증 실패, 미지원 method, protocol/schema drift, 음수 값을 정해진 error code로 변환한다.
...
- 로그인된 Codex CLI의 App Server를 shell 없이 실행하고 15초 timeout과 1MiB 출력 한도를 적용한다.
- `initialize`에 `experimentalApi: true`를 전달한 뒤 `initialized`, `account/usage/read` 순서로 호출한다.
- 실패 시 기존 profile snapshot은 유지하고 명령은 비영 종료한다.
- `dailyUsageBuckets`의 날짜 중복·비정렬·부분 오류를 통째로 거절하고, total-only인 Codex profile의 token breakdown/session은 unknown으로 유지한다.
...
- render는 공개 JSON만 읽고 로컬 CLI 인증 상태나 로컬 로그를 요구하지 않는다.
...
- Codex profile adapter가 실험적 App Server method를 사용하며 ChatGPT 로그인 전제와 local fallback을 명시한다.
...
Run when a recent Codex CLI is available and signed in with ChatGPT: `npm run profile`

Expected: 성공 시 sanitized `data/profiles/<opaque-device-id>.json`; CLI가 없거나 App Server method/protocol이 바뀌었으면 기존 candidate를 보존하고 local Codex fallback 사용.
