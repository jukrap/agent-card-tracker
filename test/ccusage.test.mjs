...
test('Codex daily 결과를 공개 day record로만 정규화한다', async () => {
  ...
  assert.equal('reasoningOutputTokens' in days[0], false);
  assert.equal('models' in days[0], false);
  assert.equal(days[0].totalTokens, 1150, 'reasoning을 포함한 upstream totalTokens를 신뢰한다');
});
...
test('session 수집 실패는 daily 수집을 버리지 않고 sessions를 unknown으로 둔다', async () => {
  ...
  const result = await collectCcusage({
    ...
    runner: async (args) => {
      ...
      return fixture('codex-daily.json');
    },
  });
  ...
  assert.deepEqual(calls, [
    buildCcusageArgs('daily', 'Asia/Seoul'),
    buildCcusageArgs('session', 'Asia/Seoul'),
  ]);
...
test('고정된 안전 옵션으로 Codex 전용 argv를 만든다', () => {
  assert.deepEqual(buildCcusageArgs('daily', 'Asia/Seoul'), [
    'codex',
    'daily',
    '--json',
    '--offline',
  ...
  assert.deepEqual(buildCcusageArgs('session', 'UTC'), [
    'codex',
    'session',
    '--json',
    '--offline',
  ...
  assert.throws(() => buildCcusageArgs('weekly', 'UTC'));
});
...
test('runner는 process.execPath와 로컬 ccusage JS를 shell 없이 제한 실행한다', async () => {
  ...
  const stdout = await runner(['codex', 'daily', '--json'], {
    env: {
