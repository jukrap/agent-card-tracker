...
test('allowlist에 없는 flat SVG 카드도 거절한다', async (t) => {
  const cwd = await createTempDirectory(t);
  await writePublicFile(cwd, 'cards/not-listed.svg', validSvg());
  await assertRepositoryError(
    () => validateRepository({ cwd }),
    'PUBLIC_CARD_PATH',
    'cards/not-listed.svg',
  );
});
...
test('오류와 CLI 출력은 실제 secret이나 absolute path를 echo하지 않는다', async (t) => {
  ...
  await writePublicFile(cwd, 'cards/overview-midnight.svg', validSvg(`<text x="12" y="24">${secret}</text>`));

  await assert.rejects(() => validateRepository({ cwd }), (error) => {
    assert.equal(error.code, 'PUBLIC_BEARER');
    assert.doesNotMatch(error.message, /do-not-echo|agent-card-validate-/i);
    assert.equal(error.path, 'cards/overview-midnight.svg');
    return true;
  });
  ...
  assert.match(output.stderr, /Validation failed: PUBLIC_BEARER at cards[/\\]overview-midnight\.svg/);
  ...
  assert.match(helpCapture.output().stdout, /Usage: codex-renown validate/);
